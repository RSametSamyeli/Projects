...
$unx_seo 	 = unserialize($sef_haberler['seo']);
$sayfa = intval($get3);
...
$kactane = $conn -> query("SELECT * FROM haber");
$num = $kactane-> rowCount();
$kacar = $def['habersayfalama'];
$sayfasayisi = ceil($num/$kacar);
$nerden 	  = ($sayfa*$kacar)-$kacar;	
$haberler     = $conn -> query("SELECT * FROM haber ORDER BY sira ASC LIMIT ".$nerden.", ".$kacar."");
$yanhaberler  = $conn -> query("SELECT * FROM haber ORDER BY sira ASC");
				...
				<div class="col-md-8 col-xs-12">
					<div class="cihaniriboy-inner-left">
						<div class="cihaniriboy-page-title">
							<h1><?php echo $lang['genel']['haberler']; ?></h1>
						</div>
						<div class="cihaniriboy-haberler-content cihaniriboy-img-list">
							<div class="row">
								...
								if($def['habertema'] == "small") {
							?>
								<div class="cihaniriboy-haber-item wow fadeIn" data-wow-delay="<?php echo $i; ?>s">
									<div class="col-sm-5">
										<div class="cihaniriboy-haber-image">
											<img src="<?php echo $set['siteurl']; ?>/uploads/haberler/thumb/<?php echo $anaresim; ?>" alt="<?php echo $baslik[$set['lang']['active']]; ?>" />
											<div class="ci-haber-hover">
												<div class="ci-buttons">
													 <a href="<?php echo $set['siteurl']; ?>/uploads/haberler/large/<?php echo $anaresim; ?>" alt="<?php echo $baslik[$set['lang']['active']]; ?>" class="cihaniriboy-u-zoom"><i class="fa fa-search"></i></a>
													 <a href="<?php echo $set['langurl']; ?>/<?php echo $detaysef_haber_link[$set['lang']['active']]; ?>/<?php echo $sef[$set['lang']['active']]."-".$haber['id']; ?>"><i class="fa fa-link"></i></a>
												</div>
											</div>
										</div>
									</div>
									<!--/habersol-->
									<div class="col-sm-7">
										<div class="cihaniriboy-haber-sag">
											<div class="cihaniriboy-haber-sag-title">
												<h4><a href="<?php echo $set['langurl']; ?>/<?php echo $detaysef_haber_link[$set['lang']['active']]; ?>/<?php echo $sef[$set['lang']['active']]."-".$haber['id']; ?>"><?php echo $baslik[$set['lang']['active']]; ?></a></h4>
											</div>
											...
											<div class="cihaniriboy-haber-link">
												<a href="<?php echo $set['langurl']; ?>/<?php echo $detaysef_haber_link[$set['lang']['active']]; ?>/<?php echo $sef[$set['lang']['active']]."-".$haber['id']; ?>"><i class="fa fa-angle-right"></i> <?php echo $lang['yardimci']['devami']; ?> </a>
											</div> 
										</div>
									</div>
									<!--/habersag-->
					...
					<div class="cihaniriboy-sayfalama">
						<div class="cihaniriboy_sayfalama_orta">
							<?php
								if($sayfa > 1){
								$onceki = $sayfa-1;
								echo '<a href="'.$set['langurl'].'/'.$sef_haber_link[$set['lang']['active']].'/'.$lang['yardimci']['sayfa'].'/'.$onceki.'"><i class="fa fa-chevron-left"></i></a>';
								}
								for($i=1; $i<=$sayfasayisi; $i++){
									if($i == $sayfa) {
									echo '<span class="cihaniriboy-spanaktif">'.$i.'</span>';
									}else{
									echo '<a href="'.$set['langurl'].'/'.$sef_haber_link[$set['lang']['active']].'/'.$lang['yardimci']['sayfa'].'/'.$i.'">'.$i.'</a>';
								}
							
							}
							if($sayfa != $sayfasayisi){
								$sonraki = $sayfa+1;
								echo '<a href="'.$set['langurl'].'/'.$sef_haber_link[$set['lang']['active']].'/'.$lang['yardimci']['sayfa'].'/'.$sonraki.'"><i class="fa fa-chevron-right"></i></a>';
							}
							?>
						</div>
					</div>
						...
						<div class="cihaniriboy-inner-sidebar-blue">
							...
							<div class="cihaniriboy-sidebar">
								<ul class="cihaniriboy-sidefirstul">
									<li class="li-open"><a href="javascript:;"><?php echo $lang['genel']['haberler']; ?></a>
										<ul class="cihaniriboy-sideul cihaniriboy-sideul-open">
											...
											?>	
											<li><a href="<?php echo $set['langurl']; ?>/<?php echo $detaysef_haber_link[$set['lang']['active']]; ?>/<?php echo $sef[$set['lang']['active']]."-".$row[0]?>"><?php echo $name[$set['lang']['active']]; ?></a></li>
											<?php } ?>
										</ul>
									</li>
