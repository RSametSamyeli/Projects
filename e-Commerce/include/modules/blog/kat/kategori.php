...
$kactane = $conn -> query("SELECT * FROM blog WHERE katid = '".intval($bul['id'])."'");
$num = $kactane-> rowCount();
$kacar = 12;
$sayfasayisi = ceil($num/$kacar);
$nerden = ($sayfa*$kacar)-$kacar;
$haberler = $conn -> query("SELECT * FROM blog WHERE katid = '".intval($bul['id'])."' ORDER BY sira ASC LIMIT ".$nerden.",".$kacar."");
$yanhaberler  = $conn ->  query("SELECT * FROM blog WHERE katid = '".intval($bul['id'])."'");
					...
					<div class="cihaniriboy-sayfalama">
						<div class="cihaniriboy_sayfalama_orta">
							<?php
								if($sayfa > 1){
								$onceki = $sayfa-1;
								echo '<a href="'.$set['langurl'].'/'.$sef_blog_link[$set['lang']['active']].'/'.$unx_sef[$set['lang']['active']].'-'.$bul['id'].'/'.$lang['yardimci']['sayfa'].'/'.$onceki.'"><i class="fa fa-chevron-left"></i></a>';
								}
								for($i=1; $i<=$sayfasayisi; $i++){
									if($i == $sayfa) {
									echo '<span class="cihaniriboy-spanaktif">'.$i.'</span>';
									}else{
									echo '<a href="'.$set['langurl'].'/'.$sef_blog_link[$set['lang']['active']].'/'.$unx_sef[$set['lang']['active']].'-'.$bul['id'].'/'.$lang['yardimci']['sayfa'].'/'.$i.'">'.$i.'</a>';
								}
							
							}
							if($sayfa != $sayfasayisi){
								$sonraki = $sayfa+1;
								echo '<a href="'.$set['langurl'].'/'.$sef_blog_link[$set['lang']['active']].'/'.$unx_sef[$set['lang']['active']].'-'.$bul['id'].'/'.$lang['yardimci']['sayfa'].'/'.$sonraki.'"><i class="fa fa-chevron-right"></i></a>';
							}
							?>
						</div>
					</div>
