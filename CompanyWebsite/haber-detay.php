<?php 
ob_start();
session_start();
require_once 'panel/netting/baglan.php';
require_once 'panel/app/ponki.php';

$iceriksor=$db->prepare("SELECT * from icerik where icerik_id=:icerik_id");
$iceriksor->execute(array(
	'icerik_id' => $_GET['icerik_id']
));

$a=$icerikcek['icerik_ad'];

$icerikcek=$iceriksor->fetch(PDO::FETCH_ASSOC);

$title=$icerikcek['icerik_ad'];

$ozet=substr($icerikcek['icerik_ozet'], 0,280);

$anahtar=$icerikcek['icerik_keyword'];

$resim=$icerikcek['icerik_resimyol'];

$link=$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];



include 'header.php'; 

?>
<div role="main" class="main">
	<section class="page-header mb-0">
		<div class="container">
			<div class="row align-items-center">
				<div class="col-md-6 text-md-left">
					<h1 class="font-weight-bold"><?php echo $icerikcek['icerik_ad']; ?></h1>
				</div>
				<div class="col-md-6">
					<ul class="breadcrumb justify-content-md-end">
						<li><a href="index.php">Anasayfa</a></li>
						<li><a href="index.php">Haberler</a></li>
						<li class="active"><?php echo $icerikcek['icerik_ad']; ?></li>
					</ul>
				</div>
			</div>
		</div>
	</section>
	<div class="container">
		<div class="row">

			<div class="col-md-12 col-lg-12 order-1 mb-5 mb-md-0 mt-5">
				<article class="blog-post mb-4">
					<header class="blog-post-header mb-3">
						<img src="<?php echo $icerikcek['icerik_resimyol']; ?>" class="img-fluid" alt="" />
					</header>

					<div class="d-flex mb-3 text-center">
						<span class="post-date text-color-primary pr-3 text-uppercase"><?php echo $icerikcek['icerik_zaman']; ?></span>
					</div>

					<p><strong>Tarih:</strong> <?php echo $icerikcek['icerik_zaman']; ?>
						<?php echo $icerikcek['icerik_detay']; ?></p>


						<footer class="blog-post-footer border border-left-0 border-right-0 py-4 mt-5">
							<div class="row justify-content-between align-items-center">
								<div class="col-12 col-sm-auto mb-3 mb-sm-0 mb-md-3 mb-lg-0">
									<ul class="list-inline mb-0">
										<?php 

										$etiketler=explode(', ',$icerikcek['icerik_keyword']); 



										foreach ($etiketler as $etiketbas) {?>
										<li class="list-inline-item"><a href="#" class="badge badge-light badge-sm badge-pill px-3 py-2"><?php echo $etiketbas; ?></a></li>
										<?php }         
										?>
									</ul>
								</div>
							</div>
						</footer>
					</article>
				</div>
			</div>
		</div>

	</div>


	<?php require_once 'footer.php'; ?>